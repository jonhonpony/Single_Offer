#![cfg(test)]
extern crate std; // std kütüphanesini kullanmak için ekledik. bu kütüphane Rust'ın standart kütüphanesidir ve Rust'ın temel işlevselliğini sağlar. creates.io dan kütüphaneleri inceleyebilirisiniz.

use crate::{token, SingleOfferClient};
use soroban_sdk::{
    symbol_short,
    testutils::{Address as _, AuthorizedFunction, AuthorizedInvocation}, // testutil içindeki Address'ı _ olarak kullanmak istediğimizi belirttik. 
    Address, Env, IntoVal, Symbol,
};

fn create_token_contract<'a>( // burada ki 'a lifetime'ı belirtir. Lifetime, Rust'ta referansların ne kadar süreyle geçerli olduğunu belirtir. meseala burada 'a lifetime'ı 'Env' struct'ı ile aynı süreyle geçerli olmasını belirtir.
    e: &Env,
    admin: &Address,
) -> (token::Client<'a>, token::StellarAssetClient<'a>) {
    let sac = e.register_stellar_asset_contract_v2(admin.clone());// register_stellar_asset_contract_v2 fonksiyonunu kullanarak yeni bir stellar asset contract oluşturduk. buradak i admin clone ile kopyaladık. 
    (
...
fn test() {
    ...
    // Bu kısım, satıcı, alıcı ve teklif kontratının token bakiyelerini kontrol eder. İşlemler sonucunda beklenen bakiyelerin doğru olup olmadığını doğrular.
    //alıcı 10 buy_token alırken, satıcı 10 sell_token alır.
